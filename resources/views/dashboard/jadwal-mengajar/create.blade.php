<x-layout.dashboard title="Tambah Jadwal Mengajar">
    <!-- Flash Messages -->
    <x-ui.flash-messages />
    ...
    <x-ui.card>
        <x-slot name="header">
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-800">Tambah Jadwal Mengajar</h3>
        ...
        <p class="text-gray-600 mb-6">Buat jadwal mengajar baru untuk penugasan mengajar yang ada.</p>

        <form action="{{ route('dashboard.jadwal-mengajar.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <x-form.select name="penugasan_mengajar_id" required placeholder="Pilih Penugasan Mengajar">
                    ...
                    @foreach ($penugasanMengajars as $penugasan)
                        <option value="{{ $penugasan->id }}"
                            {{ old('penugasan_mengajar_id') == $penugasan->id ? 'selected' : '' }}>
