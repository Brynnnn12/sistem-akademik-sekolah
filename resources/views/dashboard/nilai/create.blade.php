<x-layout.dashboard title="Tambah Komponen Nilai">
    <x-ui.card>
        <x-slot name="header">
            <h3 class="text-lg font-semibold text-gray-800">Tambah Komponen Nilai</h3>
            <x-ui.button variant="outline" icon="fas fa-arrow-left" onclick="location.href='{{ route('nilai.index') }}'">
                Kembali
            </x-ui.button>
        </x-slot>
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                    ...
                    <span class="text-blue-900">{{ $penugasan->mataPelajaran->nama }}</span>
                </div>
                <div>
                    <span class="font-medium text-blue-700">Kelas:</span>
                    <span class="text-blue-900">{{ $penugasan->kelas->nama }}</span>
                </div>
                <div>
                    <span class="font-medium text-blue-700">Tahun Ajaran:</span>
                    <span class="text-blue-900">{{ $penugasan->tahunAjaran->nama }}</span>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('nilai.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <x-form.select name="jenis" required>
                    ...
                    <option value="uts" {{ old('jenis') == 'uts' ? 'selected' : '' }}>Ujian Tengah Semester (UTS)
                    </option>
                    <option value="uas" {{ old('jenis') == 'uas' ? 'selected' : '' }}>Ujian Akhir Semester (UAS)
                    </option>
                </x-form.select>
            ...
            <div>
                ...
                <x-form.input type="number" name="bobot" placeholder="Contoh: 20" value="{{ old('bobot') }}"
            ...
            <input type="hidden" name="penugasan_mengajar_id" value="{{ $penugasan->id }}">
